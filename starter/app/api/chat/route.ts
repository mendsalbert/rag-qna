...
// TODO (Step 4): Uncomment when wiring up RAG streaming
...
export async function POST(request: NextRequest) {
  try {
    ...
    // TODO (Step 4): Wire up the RAG chain and stream grounded answers
    //
    ...
    //         if (typeof chunk.answer !== 'string' || !chunk.answer) continue
      ...
      start(controller) {
        controller.enqueue(
          new TextEncoder().encode(
            '**Starter mode:** Complete Step 4 in `README.md` to enable RAG chat.\n\nReference: `.solution/app/api/chat/route.ts`',
          ),
        )
        controller.close()
      },
